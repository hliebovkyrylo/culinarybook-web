...
export const SignInForm = () => {
  ...
  const [signIn, { isLoading: isSignInLoading, isSuccess }] =
    useSignInMutation();
  ...
  const { passwordInputType, togglePasswordVisibility } =
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2">
      ...
      <div>
        <Link className="link-text text-sm" href={"/forgot-password"}>
          {t("forgot-password")}
        </Link>
      </div>
      <Button
