...
export const VerifyAccountForm = () => {
  ...
  const onSubmit = useCallback(() => {
    verifyAccount({ code: otp }).unwrap().then(() => {
      router.push('/');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      ...
      <button type="button" onClick={onClickSignOut} className="absolute left-6 bottom-6 flex items-center text-[#727272] hover:text-[#a3a3a3]">{t('logout-button-confirm')} <ArrowRightSquare className=" ml-3" /></button>
    </form>
  )
}