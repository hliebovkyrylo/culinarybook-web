...
export const SettingsUpdateUserForm = ({ user }: { user?: IUserMe }) => {
  ...
  const onSubmit = useCallback(
    async (values: UpdateUserFormData) => {
  ...
  const handleBackgroundImageChange = handleImageChange(
  ...
  return (
    <form
      ...
      className="flex flex-col justify-between h-[85%] my-4"
    >
      <div>
        <SettingsUserPanel
        ...
        <p className={`text-red-500 text-sm ${!errors.username && "hidden"}`}>
        ...
        <Input
          ...
          className="border-[1px] border-[#313131] mb-2"
        />
        ...
        <div className="py-6 border-y-[1px] border-[#313131]">
          <div className="flex justify-between">
            ...
            <Checkbox isChecked={checked} onClick={handleCheck} />
          </div>
        ...
        <button className="w-full max-w-[220px] mt-2 h-8 rounded-md dark:bg-bg-c-3 bg-bg-c-8 hover:opacity-90 transition-all">
          <Link
            ...
            className="flex items-center justify-center text-[#727272]"
          >
            {t("title-change")}
      ...
      <Button
        text={t("save-button")}
        state={
          isLoadingUpdateUser || isLoadingUploadingImage
            ? "loading"
            : isValid
            ? "default"
            : "disabled"
