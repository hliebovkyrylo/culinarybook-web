...
export const ProfileNavigationPanel = ({
  ...
  return (
    <nav className={`w-full flex justify-between py-1 px-3 max-[409px]:dark:bg-[#171818] max-[409px]:bg-bg-c-8 rounded-xl ${className}`}>
      ...
      {(pathname === (`/profile/${userId}`) || pathname === `/profile/${userId}?sortBy=${currentSort}`) && (
        <button onClick={handleChangeSort}>
          {sortBy === 'desc' ? (
            <div className="flex gap-3">
              <ArrowSortDownIcon className="w-6 h-6 fill-color-666" />
              <p className="block text-color-666 max-sm:hidden">{t('sort-new-old')}</p>
            </div>
          ) : (
            <div className="flex gap-3">
              <ArrowSortUpIcon className="w-6 h-6 fill-color-666" />
              <p className="text-color-666 max-sm:hidden">{t('sort-old-new')}</p>
            </div>
          )}
        </button>
      )}
