...
import { useCancelFollowRequestMutation, useFollowMutation, useUnfollowMutation } from "@/lib/api/followApi";
    ...
    <section>
      <div className="flex relative py-3 px-2 gap-14 max-[537px]:gap-4">
        {data?.backgroundImage && (
          <Image className=" absolute -z-50 top-0 left-0 w-full h-full object-cover opacity-20 rounded-t-lg" src={data.backgroundImage} alt="User banner" width={500} height={500} />
        )}
        ...
        <div className="w-full max-w-[390px] h-36 flex flex-col justify-around">
          ...
          <div className="flex justify-between">
            <p className="default-text max-[537px]:!text-sm text-center"><span className="link-text max-[537px]:block">{data?.recipesCount}</span> {data?.recipesCount as number > 1 || data?.recipesCount === 0 ? t('recipes') : t('recipe')}</p>
            <Link href={selfId ? `/profile/${data?.id}/followers` : '/sign-in'} className="default-text max-[537px]:!text-sm text-center"><span className="link-text max-[537px]:block">{numberOfFollowers}</span> {numberOfFollowers > 1 || numberOfFollowers === 0 ? t('followers') : t('follower')}</Link>
            <Link href={selfId ? `/profile/${data?.id}/followings` : '/sign-in'} className="default-text max-[537px]:!text-sm text-center"><span className="link-text max-[537px]:block">{numberOfFollowings}</span> {numberOfFollowings > 1 || numberOfFollowings === 0 ? t('followings') : t('following')}</Link>
          </div>
