...
export const CommentsContent = ({
  ...
  const {
    ...
    formState: { isValid: isValidReply },
    reset: resetReply,
    setValue: setReplyValue,
  } = useForm<CreateCommentReplyFormData>({
    defaultValues: {
      commentText: "",
    },
  ...
  const [createCommentReply, { isLoading: isLoadingCreatingCommentsReplies }] =
    useCreateCommentReplyMutation();

  const [deleteComment, { isLoading: isLoadingDeleteComment }] =
    useDeleteCommentMutation();
  const [deleteCommentReply, { isLoading: isLoadingDeletingCommentReply }] =
    useDeleteCommentReplyMutation();
  ...
  const onSubmitCreateCommentReply = useCallback(
    async (values: CreateCommentReplyFormData) => {
      if (!accessToken) {
        router.push("/sign-in");
      } else {
        createCommentReply({
  ...
  const isLoadingAllComments =
    isLoadingDeleteComment ||
    isLoadingCreatingCommentsReplies ||
    isLoadingDeletingCommentReply;
              ...
              return (
                <div className="mb-6" key={index}>
                  <Comment
                    ...
                    onClickReply={() => {
                      handleOpenInput(comment.id, comment.id, comment.user.id),
                        setReplyValue("commentText", "");
                    }}
                  ...
                  <div className="ml-14">
                    {openInputId === comment.id && (
                      <form
                        onSubmit={handleSubmitReply(onSubmitCreateCommentReply)}
                      >
                        <Input
                          color="default"
                          {...registerReply("commentText")}
                          type="text"
                          placeholder={t("comment-placeholder")}
                        />
                        <div className="flex justify-end mt-3">
                          <button
                            onClick={() => setOpenInputId("")}
                            className=" mr-4"
                          >
                            {t("canclel-button")}
                          </button>
                          <Button
                            text={t("reply-button")}
                            state={isValidReply ? "default" : "disabled"}
                            className="max-w-[128px] h-8 flex justify-center items-center"
                          />
                        </div>
                      </form>
                    )}
                          ...
                          return (
                            <div key={commentReply.id}>
                              <Comment
                                ...
                                onClickDeleteComment={() =>
                                  onClickDeleteCommentReply(commentReply.id)
                                }
                              />
                              {openInputId === commentReply.id && (
                                <form
                                  onSubmit={handleSubmitReply(
                                    onSubmitCreateCommentReply
                                  )}
                                  className="ml-14"
                                >
                                  <Input
