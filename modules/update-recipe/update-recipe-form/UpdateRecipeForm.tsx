...
export const UpdateRecipeForm = ({
  ...
  const onSubmit = useCallback(async (values: UpdateRecipeAndStepFormData) => {
    if (isValid) {
      ...
      updateRecipe({ ...values, id: recipe?.id as string }).unwrap().then((recipe) => {
  ...
  }, [updateRecipe, createSteps, fields, isValid, deleteStep, steps, updateSteps, stepsToDelete]);
    ...
    <section>
      <form onSubmit={handleSubmit(onSubmit)} className="my-7">
        ...
        <div className="flex gap-12 max-md:flex-col">
          <RecipeCreateCover
            recipeImage={selectedImage ? URL.createObjectURL(selectedImage) : recipe?.image as string}
