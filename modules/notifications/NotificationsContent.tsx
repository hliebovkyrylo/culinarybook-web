...
export const NotificationsContent = ({
  ...
  const [followRequestAnswear, { isLoading: isLoadingFollowRequest }] =
      ...
      {isLoading || isLoadingFollowRequest ? (
        ...
        <>
          {data && data.length > 0 ? (
                ...
                case "follow-request":
                  return (
                    <NotificationFollowRequest
                      ...
                      createdAt={notification.createdAt}
                      onClickAllowRequest={() =>
                        handleAllowRequest(notification.notificationCreator?.id)
                      }
                      onClickRejectRequest={() =>
                        handleRejectRequest(
                          notification.notificationCreator?.id
                        )
                      }
                    />
                  );
            ...
            <p className="flex justify-center items-center h-full text-[#757575]">
              {t("no-notifications")}
            </p>
          )}
        </>
